...
BEGIN
    CREATE TABLE dbo.historico_usuarios (
        idHistoricoUsuario INT IDENTITY(1,1) NOT NULL,
        fk_usuario INT NOT NULL,
...
BEGIN
    CREATE TABLE dbo.janela (
        idJanela INT IDENTITY(1,1) NOT NULL,
        pid INT NULL,
        titulos TEXT NULL,
        comandos TEXT NULL,
...
BEGIN
    CREATE TABLE dbo.tipo_dados (
        idTipoDados INT IDENTITY(1,1) NOT NULL,
        nome VARCHAR(45) NULL,
        fk_componente INT,
        fk_maquina INT,
...
-- tipo_alerta
IF OBJECT_ID(N'dbo.tipo_alerta', N'U') IS NULL
BEGIN
    CREATE TABLE dbo.tipo_alerta (
        idTipo_alerta INT PRIMARY KEY IDENTITY(1,1) NOT NULL,
        nome VARCHAR(45) NULL,
        cor VARCHAR(6) NULL
    );
END;
...
BEGIN
    CREATE TABLE dbo.limites (
        ...
        fk_tipoComponente INT NOT NULL,
        PRIMARY KEY (fk_alerta, fk_tipoComponente),
        CONSTRAINT fk_limites_fk_notificacao FOREIGN KEY (fk_alerta) REFERENCES dbo.tipo_alerta (idTipo_alerta) ON DELETE CASCADE ON UPDATE CASCADE,
...
AS
BEGIN 
    SELECT limite FROM limites WHERE fk_alerta = @idNotVar AND fk_tipoComponente = @idTipoComVar;
END;
...
AS
BEGIN 
    SELECT m.stt_maquina as stt, m.nome as maquina, s.nome as sala, a.verificar as verificado FROM 
...
INSERT INTO limites (fk_alerta, fk_tipoComponente) VALUES (1, 1), (1, 2), (1, 3), (2, 1), (2, 2), (2,3);